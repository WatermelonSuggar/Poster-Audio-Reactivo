...
function initTiles() {
  tiles = [
    // fila superior
    { x: panel.x + 34,  y: panel.y + 110, w: 88,  h: 80,  sx: 0,   sy: 0,   sw: 106, sh: 80 },
    { x: panel.x + 130, y: panel.y + 110, w: 88,  h: 80,  sx: 107, sy: 0,   sw: 106, sh: 80 },
    { x: panel.x + 226, y: panel.y + 110, w: 88,  h: 80,  sx: 214, sy: 0,   sw: 106, sh: 80 },

    // bloque medio ancho
    { x: panel.x + 130, y: panel.y + 202, w: 184, h: 80,  sx: 80,  sy: 80,  sw: 160, sh: 80 },

    // fila inferior
    { x: panel.x + 34,  y: panel.y + 294, w: 88,  h: 80,  sx: 0,   sy: 160, sw: 106, sh: 80 },
    { x: panel.x + 130, y: panel.y + 294, w: 88,  h: 80,  sx: 107, sy: 160, sw: 106, sh: 80 }
  ];
}
...
function processNewFrame() {
  // copiar frame actual al buffer circular
...
function drawFragmentedCanvases() {
  ...
  for (let t of tiles) {
    image(src, t.x, t.y, t.w, t.h, t.sx, t.sy, t.sw, t.sh);
  }
}
...
function drawInterfaceText() {
  ...
  textSize(12);
  text(
    `Umbral actual: ${thresholdSlider.value()}`,
    panel.x + 34,
    panel.y + 430
  );

  text(
    `Estilo actual: ${styleNames[styleMode]}`,
    panel.x + 34,
    panel.y + 450
  );

  text(
    `Movimiento detectado: ${lastMovementPercentage.toFixed(1)}%`,
    panel.x + 34,
    panel.y + 470
  );

  text(
    `Buffer: ${bufferFillCount}/${BUFFER_SIZE}`,
    panel.x + 34,
    panel.y + 490
  );
  ...
  textSize(13);
  text(
    "Todos los fragmentos muestran el mismo procesamiento\npero recortado desde zonas distintas de la cámara.",
    24,
    80
  );
}
