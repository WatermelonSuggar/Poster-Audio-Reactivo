...
function createUI() {
  ...
  resetBufferBtn.position(panel.x + 136, panel.y + 392);
...
function drawFragmentedCanvases() {
  ...
  for (let t of tiles) {
    ...
    // modo espejo horizontal tipo webcam
...
function drawInterfaceText() {
  ...
  text(`Movimiento detectado: ${lastMovementPercentage.toFixed(1)}%`, panel.x + 34, panel.y + 450);
  text(`Buffer: ${bufferFillCount}/${BUFFER_SIZE}`, panel.x + 34, panel.y + 470);

  let movementState = lastMovementPercentage < MOVEMENT_THRESHOLD
    ? "Leve"
    : "Moderado";

  text(`Estado de movimiento: ${movementState}`, panel.x + 34, panel.y + 490);
  text(`Estilo automático: ${styleNames[styleMode]}`, panel.x + 34, panel.y + 510);
}
